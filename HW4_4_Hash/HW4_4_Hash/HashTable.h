...
public:
	...
	void InsertLinearProbe(ItemType newItem, int collisionChain);
	void Append(int key, ItemType newItem);
	void LinearAppend(int key, ItemType newItem, int collisionChain);
...
template<class ItemType>
void HashTable<ItemType>::InsertLinearProbe(ItemType newItem, int collisionChain)
{
	if (IsFull())
	{
	...
	{
		int key = Hash(newItem);
		LinearAppend(key, newItem, collisionChain);
	}
}
...
template<class ItemType>
void HashTable<ItemType>::LinearAppend(int key, ItemType newItem, int collisionChain)
{
	if (IsFull())
	{
	...
	else
	{
		...
		{
			++collisionChain;
			LinearAppend(key + 1, newItem, collisionChain );
		}
